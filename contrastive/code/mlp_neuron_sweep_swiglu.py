"""
MLP neuron sweep adapted for SwiGLU models (Phi-3, Phi-3.5, etc.)

SwiGLU forward:  gate_up_proj(x) → chunk → SiLU(gate) * up → down_proj
- gate_up_proj weight: (2*d_inter, d_model), first d_inter rows = gate, rest = up
- gate rows are the detectors (analogous to fc1 in GELU models)
- down_proj columns are the writers (analogous to fc2)
- Neuron i fires as: SiLU(gate_w[i] @ x) * (up_w[i] @ x)

For each contrast pair:
1. Get MLP input contrastive direction
2. Find neurons whose gate row aligns with that direction
3. Check SwiGLU gating (SiLU(gate)*up: ON for A, OFF for B)
4. Read what the cleanly gated neurons detect (gate·W_U) and write (down_proj·W_U)

Usage: MODEL=microsoft/Phi-3.5-mini-instruct .venv/Scripts/python.exe contrastive/code/mlp_neuron_sweep_swiglu.py
"""
import sys
try:
...
MODEL = os.environ.get("MODEL", "microsoft/Phi-3.5-mini-instruct")
...
def get_swiglu_activations(text, layer_idx):
    """Get per-neuron gate and up activations (pre and post SiLU)."""
    ids = tok(text, add_special_tokens=False)["input_ids"]
    captured = {}

    def hook_fn(module, input, output):
        # output is gate_up_proj(x), shape (1, seq, 2*d_inter)
    ...
    gate_pre = gate_up[:d_inter]      # pre-SiLU gate values
    up_vals = gate_up[d_inter:]       # up-projection values
    gate_post = torch.nn.functional.silu(gate_pre)  # post-SiLU
    neuron_act = gate_post * up_vals  # final neuron activation
...
def find_clean_neurons(text_a, text_b, layer_idx, gate_thresh=0.1, top_k=200):
    ...
    In SwiGLU, "ON" means |SiLU(gate) * up| > threshold.
    """
    gate_up_w = model.model.layers[layer_idx].mlp.gate_up_proj.weight.detach().float()
    gate_w = gate_up_w[:d_inter, :]    # (d_inter, d_model) — detector weights
    down_w = model.model.layers[layer_idx].mlp.down_proj.weight.detach().float()
    # down_w shape: (d_model, d_inter) — column i is what neuron i writes
    ...
    # Read alignment: how much does each gate row align with the contrast?
    read_align = gate_w @ delta_in_norm.to(DEV)  # (d_inter,)
    ...
    for n in top_readers:
        n = int(n)
        # Use the full SwiGLU activation as the gating signal
        aa = float(act_a[n])
        ab = float(act_b[n])
        ga = float(gate_pre_a[n])
        gb = float(gate_pre_b[n])
        ...
        if (a_on and not b_on) or (b_on and not a_on):
            # Read: what does this gate row detect?
            read_vec = gate_w[n, :]
            ...
            # Write: what does this neuron's down_proj column write?
            write_vec = down_w[:, n]
...
# ── RUN SWEEP ─────────────────────────────────────────────────
print("\n" + "=" * 70)
print("MLP NEURON SWEEP (SwiGLU) — clean read→gate→write detectors")
print(f"Model: {MODEL}")
print("=" * 70)
